...
if (!allAuthVarsPresent) {
  if (someAuthVars) {
    console.error(
      "Error: Partial authentication configuration detected.\n" +
...
Args:
  ...
  async (args) => {
    return handleToolError(async () => {
      ...
      const nextOffset =
        result.offset + items.length < total ? result.offset + items.length : undefined;
...
Requires authentication. Set MANGADEX_CLIENT_ID and related env vars.`,
  ...
  async ({ limit, offset }) => {
    return handleToolError(async () => {
      if (readOnlyMode) {
        throw new Error(
          "Authentication required. Set MANGADEX_CLIENT_ID, MANGADEX_CLIENT_SECRET, MANGADEX_USERNAME, and MANGADEX_PASSWORD environment variables.",
        );
      }
