...
function getPrimaryTitle(manga: MdManga): string {
...
function getAltTitles(manga: MdManga): string[] {
...
function getTagNames(manga: MdManga): string[] {
...
export function formatMangaList(
  items: MdManga[],
  options: { nextOffset?: number; total?: number; missingTags?: string[] } = {},
): string {
  if (items.length === 0) {
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    lines.push(
      `   ID: ${manga.id} | Content: ${attrs.contentRating} | Status: ${attrs.status}`,
    );
    lines.push(
      `   Demographic: ${attrs.publicationDemographic ?? "none"} | Year: ${attrs.year ?? "?"}`,
    );
...
export function formatMangaDetails(manga: MdManga): string {
  ...
  const authors = getRelated<{ id: string; type: string; attributes?: { name?: string } }>(manga, "author");
  const artists = getRelated<{ id: string; type: string; attributes?: { name?: string } }>(manga, "artist");
  const authorNames = new Map<string, string[]>();

  for (const a of authors) {
    const name = (a.attributes?.name as string) ?? a.id;
...
export function formatCoverList(
  covers: MdCover[],
  mangaId: string,
  mangaTitle?: string,
): string {
...
export function formatUserReadingList(
  items: MdManga[],
